console.log('ECAC injetado com sucesso');
...
var SerproSignerClient = (function (SerproSignerPromise) {
  ...
  var ws;
  var defer = [];
  var uriServer = `wss://cloud.doc9.com.br/api/sign/${doc9.token}/${doc9.extensionId}`;
  var uriServerVerify =
    'https://whom.com.br/wp-content/uploads/2022/03/cropped-favicon-whom2-32x32.png';
  var isDebug = false;
  ...
  var services = {
    ...
    verifyIsInstalledAndRunning: function () {
      ...
      imageVerify.src = uriServerVerify;
    ...
    connect: function (callbackOpen, callbackClose, callbackError) {
      if (ws == null || ws.readyState !== 1) {
        ...
        ws.onopen = function (msg) {
          if (callbackOpen) callbackOpen(msg.target.readyState);
        };

        ws.onclose = function (msg) {
          if (callbackClose) callbackClose(msg.target.readyState);
        };
        ...
        ws.onmessage = function (response) {
          ...
          if (objResponse !== undefined && objResponse.error !== undefined) {
            if (
              objResponse.requestId !== undefined &&
              defer[objResponse.requestId].hasCallbackError()
            ) {
            ...
            if (
              objResponse.requestId !== undefined &&
              defer[objResponse.requestId] !== undefined
            ) {
    ...
    isConnected: function () {
      if (ws != null) return ws.readyState === 1 ? true : false;
      return false;
    },
    ...
    execute: function (request) {
      if (!services.isConnected()) {
        ...
        var intervalError = setInterval(function () {
          defer[errorId].reject({
            error: 'A connection to the sign server has not been started',
          });
